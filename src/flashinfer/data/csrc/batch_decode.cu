...
void BatchDecodeWithPagedKVCacheRun(TensorView float_workspace_buffer,
  ...
  const int64_t* k_cache_strides = nullptr;
  const int64_t* v_cache_strides = nullptr;
  auto k_strides = paged_k_cache.strides();
  auto v_strides = paged_v_cache.strides();
  TVM_FFI_ICHECK_EQ(k_strides.size(), v_strides.size());
  k_cache_strides = k_strides.data();
  v_cache_strides = v_strides.data();
  ...
  DISPATCH_context(
      DTypeQ, DTypeKV, DTypeO, IdType, HEAD_DIM_QK, HEAD_DIM_VO, POS_ENCODING_MODE,
      USE_SLIDING_WINDOW, USE_LOGITS_SOFT_CAP, AttentionVariant, Params, [&] {
        // REVIEW(mixed): keep side-specific strides in the descriptor even though
        // CUDA-core decode still has a single DTypeKV kernel path.
        paged_kv_t<DTypeKV, IdType> paged_kv(
            num_kv_heads, page_size, HEAD_DIM_QK, batch_size, kv_layout,
            static_cast<DTypeKV*>(paged_k_cache.data_ptr()),
            static_cast<DTypeKV*>(paged_v_cache.data_ptr()), k_cache_strides, v_cache_strides,
