...
void BatchPrefillWithPagedKVCacheRun(TensorView float_workspace_buffer,
  ...
  DISPATCH_context(
      ...
      RaggedParams, PagedParams, [&] {
        ...
        params.q = static_cast<DTypeQ*>(q.data_ptr());
        // REVIEW(mixed): K=fp8 and V=nvfp4 have different row widths, so the
        // binding must retain independent K/V strides. Symmetric tensors still
        // produce identical stride metadata.
