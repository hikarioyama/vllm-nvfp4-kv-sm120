...
def get_batch_decode_module(*args, dtype_k=None, dtype_v=None):
    # REVIEW(mixed): split K/V dtypes are keyword-only at the URI boundary so
    # old symmetric positional args keep the exact legacy cache key.
    uri = get_batch_decode_uri(*args, dtype_k=dtype_k, dtype_v=dtype_v)
    mod = _call_jit_factory(
        gen_batch_decode_module, *args, dtype_k=dtype_k, dtype_v=dtype_v
    ).build_and_load()
    plan_func = mod.plan
    run_func = mod.run
