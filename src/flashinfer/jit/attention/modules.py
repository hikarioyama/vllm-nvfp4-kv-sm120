...
# REVIEW(mixed): this source slice does not include the upstream JIT dtype maps.
# Preserve upstream entries when present, and fill the URI-safe names needed by
# mixed K=fp8/V=nvfp4 cache specialization.
filename_safe_dtype_map.setdefault(torch.uint8, "u8")
if hasattr(torch, "float8_e4m3fn"):
    filename_safe_dtype_map.setdefault(torch.float8_e4m3fn, "fp8_e4m3")
if hasattr(torch, "float8_e5m2"):
    filename_safe_dtype_map.setdefault(torch.float8_e5m2, "fp8_e5m2")
dtype_map_kv.setdefault(torch.uint8, "uint8_t")
...
def _mixed_kv_uri_suffix(
    dtype_k: Optional[torch.dtype], dtype_v: Optional[torch.dtype]
) -> str:
    if not _is_mixed_kv_dtype(dtype_k, dtype_v):
        return ""
    return (
        f"_mixk_{filename_safe_dtype_map[dtype_k]}"
        f"_mixv_{filename_safe_dtype_map[dtype_v]}"
    )


def _mixed_kv_render_kwargs(
    ...
    if not _is_mixed_kv_dtype(dtype_k, dtype_v):
        return {}
    # REVIEW(mixed): symmetric modules render exactly as before via the Jinja
    # default(dtype_kv) path; only mixed modules override DTypeK/DTypeV.
    return {
...
def get_batch_decode_uri(
    ...
    return (
        ...
        f"use_swa_{use_sliding_window}_"
        f"use_logits_cap_{use_logits_soft_cap}"
    ) + _mixed_kv_uri_suffix(dtype_k, dtype_v)
...
def gen_customize_batch_decode_module(
    ...
    kwargs |= _mixed_kv_render_kwargs(dtype_kv, dtype_k, dtype_v)
