...
def _is_fp4_kv_dtype(dtype: Optional[torch.dtype]) -> bool:
    return dtype == torch.uint8


def _is_mixed_kv_dtype(
    dtype_k: Optional[torch.dtype], dtype_v: Optional[torch.dtype]
) -> bool:
    return dtype_k is not None and dtype_v is not None and dtype_k != dtype_v


def _module_args_with_mixed_kv(
    args: Tuple[Any, ...],
    dtype_k: Optional[torch.dtype],
    dtype_v: Optional[torch.dtype],
) -> Tuple[Any, ...]:
    if _is_mixed_kv_dtype(dtype_k, dtype_v):
        # REVIEW(mixed): symmetric module args are byte-for-byte unchanged; only
        # mixed K/V precision appends side dtypes to disambiguate the JIT URI.
        return args + (dtype_k, dtype_v)
    return args


def _call_jit_factory(factory, *args, dtype_k=None, dtype_v=None):
    if not _is_mixed_kv_dtype(dtype_k, dtype_v):
        return factory(*args)
    try:
        signature = inspect.signature(factory)
    except (TypeError, ValueError):
        signature = None
    if signature is not None and "dtype_k" in signature.parameters:
        return factory(*args, dtype_k=dtype_k, dtype_v=dtype_v)
    return factory(*_module_args_with_mixed_kv(args, dtype_k, dtype_v))
...
def get_batch_prefill_module(backend, *args, dtype_k=None, dtype_v=None):
    mixed_kv = _is_mixed_kv_dtype(dtype_k, dtype_v)
    if backend == "trtllm-gen":
        ...
        # REVIEW(mixed): URI functions take side dtypes as kwargs so legacy
        # positional module args remain unchanged for symmetric cache paths.
        uri = get_batch_prefill_uri(
            backend, *args, dtype_k=dtype_k, dtype_v=dtype_v
        )
        module = _call_jit_factory(
            gen_batch_prefill_module,
            backend,
            *args,
            dtype_k=dtype_k,
    ...
    def ragged_run(
        ...
        if backend == "fa2":
            sf_args = [maybe_k_cache_sf, maybe_v_cache_sf]
            if mixed_kv:
                # REVIEW(mixed): K=fp8 has no K-SF argument in the generated
                # Params, while V=nvfp4 keeps maybe_v_cache_sf and explicit strides.
                sf_args = []
                if _is_fp4_kv_dtype(dtype_k):
                    sf_args.append(maybe_k_cache_sf)
                if _is_fp4_kv_dtype(dtype_v):
                    sf_args.append(maybe_v_cache_sf)
            ragged_run_func(
                float_workspace_buffer,
                int_workspace_buffer,
                plan_info_vec,
    ...
    def paged_run(
        ...
        if backend == "trtllm-gen":
            ...
            sf_args = [key_block_scales, value_block_scales]
            if mixed_kv:
                # REVIEW(mixed): the generated C++ paged_run keeps BOTH
                # maybe_k_cache_sf and maybe_v_cache_sf positional slots, so pass
                # both — None for the non-FP4 side (fp8 K). The kernel compile-time
                # skips the fp8 side's SF via is_fp4_type_v<DTypeK>.
                sf_args = [
                    key_block_scales if _is_fp4_kv_dtype(dtype_k) else None,
                    value_block_scales if _is_fp4_kv_dtype(dtype_v) else None,
                ]
            paged_run_func(
                float_workspace_buffer,
                int_workspace_buffer,
...
class BatchPrefillWithPagedKVCacheWrapper:
    ...
    def plan(
        ...
        if kv_data_type is None:
            kv_data_type = dtype_k if dtype_k is not None else q_data_type
        kv_data_type = canonicalize_torch_dtype(kv_data_type)
        dtype_k = (
            canonicalize_torch_dtype(dtype_k)
            if dtype_k is not None
            else kv_data_type
        )
        dtype_v = (
            canonicalize_torch_dtype(dtype_v)
        ...
        self._cached_k_data_type = dtype_k
        self._cached_v_data_type = dtype_v
        ...
        if self._jit_module is not None:
            ...
            if self._backend != "cudnn":
                ...
                self._cached_module = get_batch_prefill_module(
                    self._backend, *get_module_args, dtype_k=dtype_k, dtype_v=dtype_v
                )
    ...
    def run(
        ...
        *args
        ...
        _check_cached_qkv_data_type(
            ...
            getattr(self, "_cached_k_data_type", self._cached_kv_data_type),
            v_cache,
            getattr(self, "_cached_v_data_type", self._cached_kv_data_type),
        )
        ...
        if k_cache.dtype == torch.uint8 and key_block_scales is None:
            raise ValueError("K scale factors must be provided for NVFP4 K cache.")
        if v_cache.dtype == torch.uint8 and value_block_scales is None:
            raise ValueError("V scale factors must be provided for NVFP4 V cache.")
...
class BatchPrefillWithRaggedKVCacheWrapper:
    ...
    def run(
        ...
        *args
        ...
        if k.dtype == torch.uint8 and k_sf is None:
            raise ValueError("K scale factors must be provided for NVFP4 K cache.")
        if v.dtype == torch.uint8 and v_sf is None:
            raise ValueError("V scale factors must be provided for NVFP4 V cache.")
