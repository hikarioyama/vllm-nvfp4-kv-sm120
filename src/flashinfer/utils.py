...
def _unpack_paged_kv_cache(
    ...
    if isinstance(paged_kv_cache, tuple):
        ...
        return (
            None if paged_k_cache is None else _expand_4d(paged_k_cache, kv_layout),
            None if paged_v_cache is None else _expand_4d(paged_v_cache, kv_layout),
        )
...
def _check_cached_qkv_data_type(
    ...
    if v is not None and dtype_v is not None and v.dtype != dtype_v:
        raise ValueError(
            f"The dtype of v {v.dtype} does not match the dtype_v {dtype_v} specified in plan function."
        )
